...
@Composable
fun ScoreEntry(key: Int, item: Highscore) {
    ...
    Card {
        Row(
            ...
            Text(text = "${key + 1}.", fontSize = 20.sp)
            Text(text = item.username, fontWeight = FontWeight.Bold, fontSize = 16.sp)
...
Column (modifier = Modifier.padding(start = 16.dp)){
    ...
    Row {
        Icon(painter = painterResource(id = R.drawable.alarm), contentDescription ="", modifier = Modifier.size(16.dp))
Spacer(modifier =Modifier.width(4.dp))
        Text(text = formatTime(item.seconds)) //todo: parse to some actual time or pass timestamp
    }

        Row {
            Icon(painter = painterResource(id = R.drawable.help), contentDescription ="", modifier = Modifier.size(16.dp))
            Spacer(modifier =Modifier.width(4.dp))

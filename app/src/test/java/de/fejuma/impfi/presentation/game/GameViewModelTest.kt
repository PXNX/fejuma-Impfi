...
class GameViewModelTest {
    ...
    // This test includes running the Coroutine timeFlow and accessing private methods and properties using reflection
    @Test
    fun initializeMap() = runTest(UnconfinedTestDispatcher()) {


        // First set up the game field, as that's what we need later on. Usually you should mock calls outside a unit though.
        // As libs like mockito don't belong to Google, this was not an option for us, so we'll skip this part.
        viewModel.configure(columns, rows, mines)

        // Reflection allows you to find a classes methods via name, even if their visibility is private
        // We then make is accessible to us (because this one is private), then call it with an asterisk in front of the arguments
        // This is a spread operator, which in this case is required for varargs
        viewModel::class
        ...
        // As the time flow emits values every second, let's wait 5 seconds so that it can complete 4 of those cycles
        delay(5_000)

        // Given that it starts at 0 and delays after emitting a value, we can now assume the it should have counted up to 4
